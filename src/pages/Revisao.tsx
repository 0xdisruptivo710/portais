...
export default function Revisao() {
  ...
  return (
    <section>
      ...
      {erro && <p role="alert">Não foi possível carregar a fila de revisão: {erro}</p>}
      {carregando && <p>Carregando fila de revisão...</p>}
      {!carregando && !erro && itens.length === 0 && <p>Fila de revisão vazia, nenhum evento pendente.</p>}

      {!carregando && itens.length > 0 && (
        <div className="revisao-layout">
          <ul className="revisao-lista">
            {itens.map((item) => (
              <li key={item.id}>
                <button
                  type="button"
                  className={item.id === selecionadoId ? "selecionado" : ""}
                  onClick={() => selecionar(item.id)}
                >
                  {rotuloPortal(item.portal)}: {item.assunto ?? "sem assunto"}
                </button>
              </li>
            ))}
          </ul>
          ...
          {selecionado && (
            <div className="revisao-detalhe">
              <div className="revisao-email">
              ...
              <form className="revisao-formulario" onSubmit={completar}>
                <h2>Completar lead</h2>

                <label htmlFor="revisao-nome">Nome</label>
                <input
                  id="revisao-nome"
                  value={form.nome}
                ...
                <label htmlFor="revisao-telefone">Telefone</label>
                <input
                  id="revisao-telefone"
                  value={form.telefone}
                ...
                <label htmlFor="revisao-veiculo">Veículo</label>
                <input
                  id="revisao-veiculo"
                ...
                <label htmlFor="revisao-email">E-mail</label>
                <input
                  id="revisao-email"
                  value={form.email}
                ...
                <label htmlFor="revisao-mensagem">Mensagem do lead</label>
                <textarea
                  id="revisao-mensagem"
